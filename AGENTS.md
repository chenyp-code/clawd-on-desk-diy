...
- 隐藏桌宠（petHidden）≠ 免打扰：隐藏只收起宠物/HUD，**新到的权限请求、更新提示、完成提示照常弹气泡，这是有意设计、不要当 bug 修**；切换瞬间已 pending 的气泡会通过 `hideFloatingSurfacesForPet` 一起收起，但下一个新气泡不再被压制。**另外 petHidden 期间 `bubbleFollowPet` 配置被忽略**，所有气泡都落 work-area 右下角 (避免「桌宠看不见，气泡钉在某个神秘坐标」)。要静默气泡走 DND（见上条）。详见 `docs/project/theme-state-ui.md` State Machine 节
- **mini mode 也是 positioning 状态、不是 silence 状态**，与 petHidden 走同一套「可见/可见但贴边 ≠ 静默」原则：updater 的 `isSilentMode()` 是 `DND && !mini`（不是 `DND || mini`），mini-sleep（mini + DND）必须能弹更新气泡；completion-bubble.js 的 `syncVisibility` 完全不检查 petHidden 或 mini——completion 气泡按 `bubble-policy.js` 的 `bypassDnd: true` 一律照常显示，连 mini-alone 也不压（mini 入场过渡的 `miniTransitioning` 由 main.js / mini.js 通过 bubble 生命周期管，不在这里）。DND-alone 仍然走 defer 路径，由 `onSilentModeExit()` 在 DND 退出时拉起。详见 `src/updater.js` `isSilentMode()` 注释与 `docs/project/theme-state-ui.md` State Machine 节
