...
- **アイドル時のデスクトップ徘徊** — アクティブな AI コーディングセッションが走っておらず、Clawd をドラッグしていないとき、対応テーマでは Clawd が 8 方向に向かってデスクトップを歩き回ります(歩き約 3.5 秒、停止約 2.5 秒、ループ)。デスクトップの他の場所でのマウス移動では止まらず、セッション開始または Clawd をつかんだときだけ停止します。`Settings...` → `General` → `Idle desktop roaming` でオフにできます（既定はオン）
