...
- **アイドル時のデスクトップ徘徊** — マウスが約 20 秒静止し AI コーディングセッションも動いていないとき、対応テーマでは Clawd が 8 方向に向かってデスクトップを歩き回ります。マウスを少しでも動かすと即座に停止します。`Settings...` → `General` → `Idle desktop roaming` でオフにできます（既定はオン）
