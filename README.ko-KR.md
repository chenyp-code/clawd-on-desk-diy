...
- **유휴 데스크톱 로밍** — 마우스가 약 20초간 정지하고 활성 AI 코딩 세션이 없으면, 지원하는 테마가 Clawd를 8 방향으로 데스크톱 위를 걷게 합니다. 마우스를 조금이라도 움직이면 즉시 멈춥니다. `Settings…` → `General` → `Idle desktop roaming`에서 끌 수 있습니다(기본 켜짐).
