...
- **유휴 데스크톱 로밍** — 활성 AI 코딩 세션이 없고 Clawd를 드래그하고 있지 않을 때, 지원하는 테마는 Clawd를 8 방향으로 데스크톱 위를 걷게 합니다(걷기 약 3.5초, 멈춤 약 2.5초, 반복). 데스크톱 다른 곳의 마우스 움직임은 멈추게 하지 않으며, 세션이 시작되거나 Clawd를 잡을 때만 멈춥니다. `Settings…` → `General` → `Idle desktop roaming`에서 끌 수 있습니다(기본 켜짐).
