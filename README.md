...
- **Idle desktop roaming** — when no live AI session is running and you're not dragging the pet, supported themes walk Clawd around your desktop in 8 directions (~3.5 s walking, ~2.5 s paused, looping). Mouse movement elsewhere does not cancel the walk — only starting a session or grabbing the pet does. Toggle via `Settings…` → `General` → `Idle desktop roaming` (default on)
